...
$sql= "SELECT Name FROM product Where Adress='buff.php'";
$result = mysqli_query($db, $sql);
if( $row = mysqli_fetch_assoc($result)){
    $productname = $row['Name'];
}
$sql= "SELECT Description FROM product Where Adress='buff.php'";

$result = mysqli_query($db, $sql);
if( $row = mysqli_fetch_assoc($result)){
    $productdes = $row['Description'];
}
$sql= "SELECT Price FROM product Where  Adress='buff.php'";

$result = mysqli_query($db, $sql);
if( $row = mysqli_fetch_assoc($result)){
    $productprice = $row['Price'];
}
$sql= "SELECT Product_id FROM product Where  Adress='buff.php'";
$result = mysqli_query($db, $sql);
if( $row = mysqli_fetch_assoc($result)){
    $productid = $row['Product_id'];
}
$sql= "SELECT Statue FROM product Where  Adress='buff.php'";
$result = mysqli_query($db, $sql);
if( $row = mysqli_fetch_assoc($result)){
	 ...
	 <div class="col-md-8 fashions2">			 
			<div class="slider2">
				<ul class="rslides rslider" id="slider2">				  
				  <li><img src="images/shawls&cowls/buff1.png" alt=""></li>
